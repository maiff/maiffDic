...
sideDict.showPop = function(type,e){
	...
	*/
	...
	$.get(http_s+"://api.bing.com/qsonhs.aspx?mkt=zh-CN&ds=bingdict&q="+queryWord,"json",function(_result){
		...
		//add phonetic symbol
		$.get('https://www.maiff.cn/lookup?q='+queryWord,'json',function(data){
			data.yodaodict['phonetic-symbol'] && data.yodaodict['phonetic-symbol'].forEach(function(ele) {
			  _title += `<span>[${ele}]</span>`
			});
			
			$("#hldict-pop-content").prepend('<div id="hldict-pop-title">'+_title+'</div>');
		})
		var _content = "";
		var _storage = "";
		if(_result.AS.FullResults){
			...
			$("#hldict-pop-content").html('<div id="hldict-pop-detail">'+_content+'</div><div id="hldict-pop-more"><span id="add-dic">add</span><span id="add-status"></span>More</div>')
		}else{
		...
		$('#add-dic').click(function(e){
			chrome.storage.sync.get("cookie", function(items){
				let cookie = items.cookie
				console.log(typeof cookie)
				chrome.storage.local.get(["words"],function(items){
					var _words = items["words"][items["words"].length - 1]
					// console.log(_words)
					$.post('https://www.maiff.cn/add', {
						cookie: cookie + '',
...
sideDict.storage = function(_word, _translation, _url, _important , _s){
	var _time = new Date();
	_time = _time.getTime();
	_entry = {
		w : _word,
